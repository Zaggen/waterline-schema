...
JoinTables.prototype.buildJoins = function(collectionName) {
  var self = this;
  var tables = [];

  var attributes = this.collections[collectionName].attributes;
  ...
  collectionAttributes.forEach(function(attribute) {
    // Unless useTableName: true is specified on the association, we will use the collectionName
    identity = (attribute.useTableName) ? this.collections[collectionName].tableName : collectionName;

    var table = self.parseAttribute(identity, attribute);
...
JoinTables.prototype.findPrimaryKey = function(collectionName) {
  ...
  if(!this.collections[collectionName]) {
    throw new Error('Trying to access a collection ' + collectionName + ' that is not defined.');
  }

  if(!this.collections[collectionName].attributes) {
    throw new Error('Collection, ' + collectionName + ', has no attributes defined.');
  }

  for(var key in this.collections[collectionName].attributes) {
    attribute = this.collections[collectionName].attributes[key];
...
JoinTables.prototype.linkAttributes = function() {
  ...
  for(var collectionName in this.collections) {
    var attributes = this.collections[collectionName].attributes;
    this.updateAttribute(collectionName, attributes);
  }
...
JoinTables.prototype.updateAttribute = function(collectionName, attributes) {

  for(var attribute in attributes) {
    if(!hop(attributes[attribute], 'collection')) continue;

    var attr = attributes[attribute];
    var parent = (attributes[attribute].useTableName) ? this.collections[collectionName].tableName : collectionName;;
    ...
    if(!hop(joined.table.attributes, parent + '_' + attribute)) continue;

    this.collections[collectionName].attributes[attribute] = {
      collection: joined.table.identity,
      references: joined.table.identity,
      on: joined.table.attributes[parent + '_' + attribute].columnName,
      onKey: joined.table.attributes[parent + '_' + attribute].columnName
    };
  }
